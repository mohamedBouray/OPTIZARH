...
<body>
        ...
        <table class="stats-grid">
            <tr>
                ...
                <td class="stat-card">
                    ...
                    <div class="stat-value">{{ $data->roles->sum(function ($r) {
            return $r->grades->count(); }) }}</div>
                </td>
                <td class="stat-card">
                    <div class="stat-label">Masse Salariale Moy.</div>
                    <div class="stat-value">
                        {{ number_format($data->roles->avg(function ($r) {
            return $r->grades->avg(function ($g) {
                return $g->echelles->avg(function ($e) {
                    return $e->echelons->avg('salary'); }); }); }), 0, ',', ' ') }}
                        <small>MAD</small></div>
                </td>
            </tr>
        </table>
                                ...
                                @foreach($echelle->echelons as $echelon)
                                    <tr>
                                        @if($loop->parent->first && $loop->first)
                                            <td rowspan="{{ $grade->echelles->sum(function ($e) {
                                                    return $e->echelons->count(); }) }}"
                                                class="grade-row">
                                                {{ $grade->name }}
                                            </td>
                                        @endif
    ...
    @else
        <div style="text-align: center; padding: 50px; color: #94a3b8; border: 2px dashed #e2e8f0; border-radius: 12px;">
            <p style="font-size: 14px;"> Aucune donnée disponible pour l'année {{ $year }}</p>
        </div>
    @endif
